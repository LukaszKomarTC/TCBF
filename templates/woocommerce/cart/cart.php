		...
		<tbody>
			...
			foreach ( $tcbf_ungrouped as $cart_item_key => $cart_item ) :
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					$product_permalink = apply_filters( 'woocommerce_cart_item_permalink', $_product->is_visible() ? $_product->get_permalink( $cart_item ) : '', $cart_item, $cart_item_key );
					?>
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>" data-cart_item_key="<?php echo esc_attr( $cart_item_key ); ?>">
					...
					<?php
					// Standalone EB footer (for booking products with EB)
					$eb_base_standalone = \TC_BF\Integrations\WooCommerce\Woo_OrderMeta::get_cart_item_eb_base_public( $cart_item );
					if ( $eb_base_standalone > 0 ) :
						$qty_s = max( 1, (int) $cart_item['quantity'] );
						$base_s = $eb_base_standalone * $qty_s;
						$eb_pct_s = 0;
						$eb_amt_s = 0;
							...
							<td colspan="6" class="tcbf-pack-footer-cell">
								<div class="tcbf-pack-footer tcbf-pack-footer--cart">
									<div class="tcbf-pack-footer-line tcbf-pack-footer-base">
										...
										<span class="tcbf-pack-footer-value"><?php echo wp_kses_post( wc_price( $base_s ) ); ?></span>
									</div>
									<div class="tcbf-pack-footer-line tcbf-pack-footer-eb-row">
										<span class="tcbf-eb-badge tcbf-cart-eb-badge">
											<?php
										...
										<span class="tcbf-pack-footer-value tcbf-pack-footer-total-value"><?php echo wp_kses_post( wc_price( $total_s ) ); ?></span>
									</div>
								</div>
							</td>
						</tr>
